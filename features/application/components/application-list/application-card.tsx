...
export default function ApplicationCard({
  ...
  const cleanCompanyName = useMemo(() => {
    ...
    let clean = companyName.replace(/\b(Inc\.?|L\.?L\.?C\.?|Ltd\.?|Co\.?|Corporation|Corp\.?|GmbH|S\.?A\.?)\b/gi, "").trim();
  ...
  return (
    <Card className="group relative min-w-[260px] overflow-hidden border border-border/60 bg-card transition-all duration-300 hover:shadow-lg hover:shadow-primary/10 hover:-translate-y-0.5 hover:border-primary/30">
      <div 
        className={cn(
          "absolute inset-x-0 top-0 h-0.5 bg-linear-to-r transition-opacity duration-300",
          score >= 80
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-center gap-2.5 min-w-0 flex-1">
            ...
            <div className="min-w-0 flex-1">
              <h3 className="font-semibold text-foreground truncate leading-tight" title={companyName}>
                {cleanCompanyName}
              </h3>
            </div>
          </div>
